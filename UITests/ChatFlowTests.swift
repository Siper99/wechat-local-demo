...
final class ChatFlowTests: XCTestCase {
    ...
    func testMainPagesAndSearch() {
        ...
        // 右上角 ＋：深色浮层菜单
        app.buttons["chats.plus"].tap()
        XCTAssertTrue(app.buttons["plus.发起群聊"].waitForExistence(timeout: 3))
        XCTAssertTrue(app.buttons["plus.添加朋友"].exists)
        capture("30-plus-menu")
        app.buttons["plus.扫一扫"].tap()
        ...
        XCTAssertTrue(app.buttons["contacts.entry.企业微信联系人"].exists)
    ...
    func testComposeQuoteFavoriteAndPanels() {
        ...
        XCTAssertTrue(app.buttons["chatInfo.search"].waitForExistence(timeout: 3))
        XCTAssertTrue(app.switches["chatInfo.remind"].exists)
        capture("11-chat-info")
        app.buttons["chatInfo.search"].tap()
        let searchField = app.textFields["chatSearch.field"]
        XCTAssertTrue(searchField.waitForExistence(timeout: 3))
        XCTAssertTrue(app.buttons["chatSearch.图片与视频"].exists)
        capture("36-chat-search")
        searchField.tap()
        searchField.typeText("Quoted")
        let result = app.buttons["chatSearch.result"].firstMatch
        XCTAssertTrue(result.waitForExistence(timeout: 3))
        result.tap()
        XCTAssertTrue(input.waitForExistence(timeout: 5))
        XCTAssertTrue(app.buttons["聊天信息"].waitForExistence(timeout: 3))
        app.navigationBars.buttons.firstMatch.tap()
    ...
    func testSwipeActionsPhotoSheetVoiceAndProfile() {
        ...
        // 点对方头像进入资料页
        let avatars = app.descendants(matching: .any).matching(identifier: "message.avatar")
        XCTAssertTrue(avatars.firstMatch.waitForExistence(timeout: 3))
        avatars.element(boundBy: avatars.count - 1).tap()
        XCTAssertTrue(app.buttons["contact.more"].waitForExistence(timeout: 3))
        XCTAssertTrue(app.staticTexts["微信号：lin_xiaoyu"].exists)
        back()
        back()

        // 通讯录 → 好友资料页
        app.buttons["tab.1"].tap()
        app.buttons["contact.row.阿杰"].tap()
        XCTAssertTrue(app.buttons["contact.more"].waitForExistence(timeout: 3))
        XCTAssertTrue(app.staticTexts["昵称：Jay"].exists)
        XCTAssertTrue(app.staticTexts["地区：上海 浦东新区"].exists)
        capture("17-contact-profile")
        // 朋友资料：改电话
        app.buttons["contact.friendInfo"].tap()
        XCTAssertTrue(app.buttons["friendInfo.phone"].waitForExistence(timeout: 3))
        app.buttons["friendInfo.phone"].tap()
        let value = app.textFields["valueEditor.field"]
        XCTAssertTrue(value.waitForExistence(timeout: 3))
        value.typeText("13800000000")
        app.buttons["valueEditor.done"].tap()
        let phone = app.buttons.matching(NSPredicate(format: "label CONTAINS %@", "13800000000")).firstMatch
        XCTAssertTrue(phone.waitForExistence(timeout: 3))
        capture("34-friend-info")
        back()
        // ··· 设置：星标、推荐给朋友
        app.buttons["contact.more"].tap()
        XCTAssertTrue(app.buttons["contactSettings.recommend"].waitForExistence(timeout: 3))
        XCTAssertTrue(app.buttons["contactSettings.delete"].exists)
        capture("33-contact-settings")
        app.buttons["contactSettings.recommend"].tap()
        XCTAssertTrue(app.navigationBars["选择聊天"].waitForExistence(timeout: 3))
        capture("37-chat-picker")
    ...
    func testMeEntriesOpen() {
        ...
        capture("20-services")
        // 钱包 → 零钱：演示充值后显示余额
        app.buttons["services.wallet"].tap()
        app.buttons["wallet.change"].tap()
        app.buttons["change.topUp"].tap()
        let amount = app.textFields["amount.field"]
        XCTAssertTrue(amount.waitForExistence(timeout: 3))
        amount.typeText("88.8")
        app.buttons["amount.confirm"].tap()
        XCTAssertEqual(app.staticTexts["change.balance"].label, "¥88.80")
        capture("31-change")
        back()
        back()
        back()
    ...
    func testContactEntriesAndGroupChat() {
        ...
        app.buttons["tab.1"].tap()
        app.buttons["contacts.entry.群聊"].tap()
        ...
        back()
        app.buttons["contacts.entry.企业微信联系人"].tap()
        XCTAssertTrue(app.navigationBars["企业微信联系人"].waitForExistence(timeout: 3))
        back()
        app.buttons["contacts.entry.新的朋友"].tap()
        ...
        // 删除申请后为空状态，“添加朋友”仍可点击（之前被空状态遮挡）
        app.cells.containing(.staticText, identifier: "周同学").firstMatch.swipeLeft()
        app.buttons["删除"].tap()
        XCTAssertTrue(app.staticTexts["newFriends.empty"].waitForExistence(timeout: 3))
        app.buttons["newFriends.add"].tap()
        XCTAssertTrue(app.navigationBars["新建联系人"].waitForExistence(timeout: 3))
        app.buttons["取消"].tap()
        back()
        app.buttons["contacts.entry.公众号"].tap()
        XCTAssertTrue(app.staticTexts["生活小百科"].waitForExistence(timeout: 3))
        back()
        app.buttons["contacts.entry.标签"].tap()
